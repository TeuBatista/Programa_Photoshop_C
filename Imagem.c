...
Imagem *alocaImagem(int largura, int altura){
    ...
    Imagem *imag=mallocSafe(sizeof(Imagem));
    imag->largura=largura;
    imag->altura=altura;
    imag->pixel= mallocSafe(altura*sizeof(int*));
    for(int i=0; i<altura; i++){
        imag->pixel[i] = mallocSafe(largura*sizeof(int));
    }
    return imag;
...
Imagem* copiaImagem(Imagem *origem){
    ...
    Imagem *imgCopy=alocaImagem(origem->largura,origem->altura);
    imgCopy->largura=origem->largura;
    imgCopy->altura=origem->altura;
    ...
    if (origem == NULL){
        ...
        for (int k=0;k<imgCopy->altura;k++){
            for(int i=0; i<imgCopy->largura; i++){
                imgCopy->pixel[k][i]=origem->pixel[k][i] ;
            }
        }
    }
    return imgCopy;
...
void salvaImagem(Imagem *img, const char *nomeArquivo){
    ...
    fprintf(arquivo, "#EP1 - Estrutura de Dados 1 - Matheus Batista\n");
